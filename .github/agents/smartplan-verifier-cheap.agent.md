...
description: Max-savings verifier (smartcheck seat) for Cheap leaves. A fresh GPT-5.6 Luna context re-runs acceptance, audits the diff for scope creep, and issues PASS/FAIL. Must be explicitly invoked. Never on shared-blind-spot classes or escalated leaves, which keep smartplan-verifier.
model: gpt-5.6-luna  # max-savings Cheap-leaf verifier, added 2026-09-16 at the author's direction ("leverage luna unless it's unsuitable"). Same model as the Cheap implementer and scout, so a verdict here is same-model review, which check.md § Family decorrelation accepts as a noted risk rather than a FAIL. Cheapest dispatchable Copilot model (0.20/1.20, cache-read 0.02), about a quarter of Gemini 3.8 Flash's input price, and above it on vals.ai SWE-V (93.0 vs 80.0). UNMEASURED as a verifier: no seeded-recall trial has compared a fresh Luna against a cross-family judge on Luna-written leaves. Run that T34-style trial on a paid plan, and move this mode back to smartplan-verifier if fresh Luna misses what the cross-family seat catches. Pin ignored/downgraded on 0x-cost-tier sessions — github/copilot-cli#2758
...
**Suitability check, before anything else.** This seat exists so max-savings can verify Cheap leaves on the cheapest model. It's unsuitable, and you must stop and report BLOCKED naming `@smartplan-verifier` as the Unblock, when any of these holds:
...
- The leaf was escalated past Cheap (fail-twice, or authored by Sonnet 5 or above). check.md's Tiering rule gives those a Strong or cross-family verifier.
